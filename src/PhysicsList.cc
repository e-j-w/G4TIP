	...
	while( (*GetParticleIterator())() ){
		...
		if (particleName == "gamma")
		...
		else if( particleName == "GenericIon")
			{
				...
				G4IonParametrisedLossModel* theModel= new G4IonParametrisedLossModel(); // ICRU 73 based model, valid for Z = 3 to 26, modified by JW to allow for custom stopping powers
				if(customStopping)
					{
						theModel->RemoveDEDXTable("ICRU73");
						theModel->AddDEDXTable("Custom",new G4IonStoppingData(cspath),new G4IonDEDXScalingICRU73()); //add stopping power data from data files
					}
