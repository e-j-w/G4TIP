...
void TrackerCsISD::Initialize(G4HCofThisEvent*){
  ...
  CsICollection = new TrackerCsIHitsCollection(SensitiveDetectorName,"CsICollection"); 
...
void TrackerCsISD::EndOfEvent(G4HCofThisEvent* HCE){
  ...
  if(NbHits>0)
		if(print){
			G4cout << "\n-------->Hits Collection: in this event there are " << NbHits	<< " hits for CsI tracking: " << G4endl;
