...
TrackerPINSD::TrackerPINSD(G4String name) : G4VSensitiveDetector(name) {
  G4String HCname;
...
void TrackerPINSD::Initialize(G4HCofThisEvent *) {
  ...
  PINCollection =
      new TrackerPINHitsCollection(SensitiveDetectorName, "PINCollection");
}
...
void TrackerPINSD::EndOfEvent(G4HCofThisEvent *HCE) {
  ...
  if (NbHits > 0)
    if (false) {
      ...
      for (i = 0; i < NbHits; i++)
        (*PINCollection)[i]->Print();
      //	   	    getc(stdin);
    }

  static G4int HCID = -1;
  if (HCID < 0) {
    HCID = G4SDManager::GetSDMpointer()->GetCollectionID("PINCollection");
  }
