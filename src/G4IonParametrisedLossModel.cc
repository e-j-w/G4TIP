...
G4bool G4IonParametrisedLossModel::AddDEDXTable(
  ...
  //G4cout << "Adding DEDX table: " << nam << G4endl;
...
G4bool G4IonParametrisedLossModel::RemoveDEDXTable(
  ...
  for(;iter != iter_end; iter++) {
     ...
     if(tableName == nam) {
        //G4cout << "Removing DEDX table: " << nam << G4endl;
        delete (*iter);
...
void G4IonParametrisedLossModel::ListDEDXTables() {

  LossTableList::iterator iter = lossTableList.begin();
  LossTableList::iterator iter_end = lossTableList.end();

  G4cout << "List of stopping tables that are being used:" << G4endl;
  for(;iter != iter_end; iter++) {
     G4String tableName = (*iter) -> GetName();
     G4cout << "   " << tableName << G4endl;
  }
}
