...
CsI_detector::CsI_detector(G4LogicalVolume *experimentalHall_log,
                           Materials *mat) {
  materials = mat;
  ...
  // CsI_absorber_Material = materials->FindMaterial("G4_Galactic");
...
G4VPhysicalVolume *CsI_detector::Construct() {
  ...
  G4Box *xCsI_detector = new G4Box(
      "xCsI", (CsI_detector_side_x - 2. * CsI_absorber_thickness) / 2.,
      (CsI_detector_side_y - 2. * CsI_absorber_thickness) / 2.,
      CsI_detector_thickness);
  aCsI_detector = new G4Box(
      "xCsI", (CsI_detector_side_x - 2. * CsI_absorber_thickness) / 2.,
      (CsI_detector_side_y - 2. * CsI_absorber_thickness) / 2.,
      CsI_detector_thickness / 2.);
  G4Box *xCsI_side_absorber =
      new G4Box("xCsIside", CsI_detector_side_x / 2., CsI_detector_side_y / 2.,
                CsI_detector_thickness / 2.);
  aCsI_absorber =
      new G4Box("CsIabsorber", CsI_detector_side_x / 2.,
                CsI_detector_side_y / 2., CsI_absorber_thickness / 2.);

  aCsI_side_absorber =
      new G4SubtractionSolid("CsIside", xCsI_side_absorber, xCsI_detector);
  CsI_detector_log =
...
void CsI_detector::Report() {
  ...
  G4cout << "----> CsI_detector side X is set to "
         << G4BestUnit(2. * aCsI_detector->GetXHalfLength(), "Length")
         << G4endl;
  G4cout << "----> CsI_detector side Y is set to "
         << G4BestUnit(2. * aCsI_detector->GetYHalfLength(), "Length")
         << G4endl;
  G4cout << "----> CsI_detector thickness is set to "
         << G4BestUnit(2. * aCsI_detector->GetZHalfLength(), "Length")
         << G4endl;
}
