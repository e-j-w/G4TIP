...
CsI_detector_spherical::CsI_detector_spherical(G4LogicalVolume* experimentalHall_log,Materials* mat)
{
  materials=mat;
  expHall_log=experimentalHall_log;
  CsI_absorber_thickness=15.0*um;
  CsI_Material = materials->FindMaterial("CesiumIodide");
  CsI_absorber_Material = materials->FindMaterial("Lead");
...
{;}
//-----------------------------------------------------------------------------
G4VPhysicalVolume* CsI_detector_spherical::Construct(G4ThreeVector pos, G4ThreeVector dim)
{
  G4double radius, thick, theta, dtheta, phi, dphi;
  radius = pos.getR();     thick  = dim.getR();
  theta  = pos.getTheta(); dtheta = dim.getTheta();
  phi    = pos.getPhi();   dphi   = dim.getPhi();
  ...
  */
  ...
  G4Sphere* xCsI_cut      = new G4Sphere("xCsIcut", 0.*mm, 2.*r2, p1+abs_deg, 2*(dphi-abs_deg), t1+abs_deg, 2*(dtheta-abs_deg));
  G4Sphere* xCsI_detector = new G4Sphere("xCsIdetector", r1, r2, p1+abs_deg, 2*(dphi-abs_deg), t1+abs_deg, 2*(dtheta-abs_deg));
  G4Sphere* xCsI_side     = new G4Sphere("xCsIside", r0, r2, p1, 2*dphi, t1, 2*dtheta);
  G4Box* xCsI_face = new G4Box("xCsI_face", r1, r1, r1);
  G4Box* xAbs_face = new G4Box("xAbs_face", r0, r0, r0);
  
  //For trapezoids
  //G4Trap* xCsI_detector = new G4Trap("xCsI", pDz+aDz, pTheta, pPhi, pDy1-aDz, pDx1-aDz, pDx2-aDz, pAlp1, pDy2-aDz, pDx3-aDz, pDx4-aDz, pAlp2);
  //G4Trap* aCsI_detector = new G4Trap("xCsI", pDz, pTheta, pPhi, pDy1-aDz, pDx1-aDz, pDx2-aDz, pAlp1, pDy2-aDz, pDx3-aDz, pDx4-aDz, pAlp2);
  //G4Trap* xCsI_side_absorber = new G4Trap("xCsIside", pDz, pTheta, pPhi, pDy1, pDx1, pDx2, pAlp1, pDy2, pDx3, pDx4, pAlp2);
  //G4Trap* aCsI_absorber = new G4Trap("CsIabsorber", aDz/2., pTheta, pPhi, aDy1, aDx1, aDx2, pAlp1, aDy2, aDx3, aDx4, pAlp2);
  
  G4Transform3D transform(Rot, G4ThreeVector(0.,0.,0.));
  G4SubtractionSolid* xCsI_side_cut      = new G4SubtractionSolid("xCsIsideCut", xCsI_side, xCsI_cut);
  G4SubtractionSolid* aCsI_side_absorber = new G4SubtractionSolid("aCsIside", xCsI_side_cut, xCsI_face, transform);
  G4SubtractionSolid* aCsI_detector      = new G4SubtractionSolid("aCsIdetector", xCsI_detector, xCsI_face, transform);
  
  //G4IntersectionSolid* xCsI_abs_cut      = new G4IntersectionSolid("xCsIabsCut", xCsI_side, xCsI_face, transform);
  //G4SubtractionSolid* aCsI_absorber      = new G4SubtractionSolid("aCsIabsorber", xCsI_abs_cut, xAbs_face, transform);
  G4SubtractionSolid* xCsI_abs_cut   = new G4SubtractionSolid("xCsIabsCut", xCsI_face, xAbs_face);
  G4IntersectionSolid* aCsI_absorber = new G4IntersectionSolid("aCsIabsorber", xCsI_side, xCsI_abs_cut, transform);
  /*/
  G4Sphere* aCsI_detector = new G4Sphere("aCsIdetector", r1, r2, p1+abs_deg, 2*(dphi-abs_deg), t1+abs_deg, 2*(dtheta-abs_deg));
  G4Sphere* xCsI_side     = new G4Sphere("xCsIside", r1, r2, p1, 2*dphi, t1, 2*dtheta);
  G4Sphere* aCsI_absorber = new G4Sphere("aCsIabsorber", r0, r1, p1, 2*dphi, t1, 2*dtheta);
  G4SubtractionSolid* aCsI_side_absorber = new G4SubtractionSolid("aCsIside", xCsI_side, aCsI_detector);
  */
