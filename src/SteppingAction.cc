...
void SteppingAction::UserSteppingAction(const G4Step *aStep) {
  ...
  particleType = 1;
  else if (aStep->GetTrack()->GetParticleDefinition()->GetParticleName() ==
  "e-")       particleType = 2;
  else if (aStep->GetTrack()->GetParticleDefinition()->GetParticleName() ==
  "e+")       particleType = 3;
  else if (aStep->GetTrack()->GetParticleDefinition()->GetParticleName() ==
  "proton")   particleType = 4;
  else if (aStep->GetTrack()->GetParticleDefinition()->GetParticleName() ==
  "neutron")  particleType = 5;
  else particleType = 0;*/
  ...
  if (edep != 0 && found != G4String::npos) {
    ...
    // eventaction->AddStepTracker(evntNb, stepNumber, cry-1, det-1, edep,
  ...
  // if (edep != 0 && found!=G4String::npos) {
  //     SetDetAndCryNumberForGriffinComponent(volname);
  //     //G4cout << "left_suppressor_extension Found Edep = " << edep/keV << "
  //     keV in det = " << det << " cry = " << cry << " found = " << found << "
  ...
  // if (edep != 0 && found!=G4String::npos) {
  //     SetDetAndCryNumberForGriffinComponent(volname);
  //     //G4cout << "left_suppressor_casing_log Found Edep = " << edep/keV << "
  //     keV in det = " << det << " cry = " << cry << " found = " << found << "
  //     volname = " << volname << G4endl;
  ...
  if (edep != 0 && found != G4String::npos) {
    ...
    //      volname = " << volname <<" weight= "<<weight<<" particle type
    //      "<<particleType<<G4endl;
    //     getc(stdin);
    eventaction->AddGriffinCrystDet(edep, weight, pos2, det - 1, cry - 1,
                                    time2);
...
void SteppingAction::SetDetAndCryNumberForGriffinComponent(G4String volname) {
  ...
  // G4cout << "Found Edep in " << volname <<  " cry = " << cry << " det = " <<
  // det << " av = " << av << G4endl;
}

void SteppingAction::SetDetAndCryNumberForDeadLayerSpecificGriffinCrystal(
    G4String volname) {
  const char *cstr = volname.c_str();
  G4int av;
  // G4int impr;
...
void SteppingAction::SetDetNumberForGenericDetector(G4String volname) {
  ...
  // G4cout << "Found electron ekin in " << volname << " det = " << det <<
  // G4endl;
}
